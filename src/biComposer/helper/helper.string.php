...
/**
 * If current page match given string print class
 * eg: use for menu highlight
 */
function addClassWhenActive($menuVoice, $class='active') {
  $pageName = basename($_SERVER['PHP_SELF']);

  if(strstr($pageName, $menuVoice)) {
    echo $class;
  }
}
...
/**
 * Output ascii encoded email
 */
function encodeMailto($address){
  $address = asciiEncode($address);
  return '<a href="mailto:'.$address.'">'.$address.'</a>';
}

/**
 * Line-Break if $string is not empty
 */
function brIf($string, $add='<br />') {
  if($string) {
    return $string.$add;
  }
}
...
/**
 * Tag-Wrap if $string is not empty
 */
function wrapIf($string, $tag='p') {
  if(!empty($string)) {
 ...
 * Tag-Wrap if $string is not empty
 */
function ddIf($string, $label='') {
  if(!empty($string)) {
    return "<dt>$label</dt><dd>$string<dd>";
  }
}


/**
 * Return ID code of youtube video url
 */
